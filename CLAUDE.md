...
Field types are `int` (1 byte) or `word` (2 bytes) for now. Element size is
the sum of field widths (declaration order, no padding). The parser records
the type table and per-array element type; `arr[i].field` is resolved into
`Stmt::StructSet` / `Expr::StructGet` nodes carrying `(field_offset,
field_width, elem_size)`, and codegen emits either an absolute LDA/STA
(constant index) or a `(ptr),Y` sequence prefixed by a shift-and-add
...
fields, multi-dimensional struct arrays.
